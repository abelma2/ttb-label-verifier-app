...
pip install pytest && pytest                  # all unit tests (pure, no network; ~170 tests, all in tests/test_verification.py)
...
- **Wine appellation is conditionally mandatory** (`_check_appellation`): a wine that names a
  grape varietal, shows a vintage, or makes an estate-bottled claim must carry an appellation
  of origin (27 CFR 4.25/4.34).
  ...
  `_COMMON_VARIETALS` (full list is 27 CFR 4.91); a confident absence fails, an unreadable read
  reviews. It's the one conditional disclosure whose trigger is visible on the label (unlike
  sulfites/FD&C #5/etc., which stay in `additional_statements`).
